...
var agentToolNames = []string{toolSpawnAgent, toolSendInput, toolWait, toolCloseAgent}

// agentPlan is the remote job's plan: which agent tool, with what.
type agentPlan struct {
	Tool      string          `json:"tool"`
	Arguments json.RawMessage `json:"arguments"`
}

// agentRegistry offers the agent tools when the session may spawn, and
// always resolves their names, so a session with past agent calls resumes
// where agents are off.
type agentRegistry struct {
	tool.Registry

	offer bool
	roles []engine.AgentRole
}

func withAgents(r tool.Registry, offer bool, roles []engine.AgentRole, disallowed []string) tool.Registry {
	return agentRegistry{Registry: r, offer: offer && !slices.Contains(disallowed, toolSpawnAgent), roles: roles}
}
...
func (r agentRegistry) StaticDefinitions() []tool.Definition {
	...
	for _, t := range []struct{ name, description, schema string }{
		{toolSpawnAgent, spawnDescription(r.roles), spawnSchema},
		{toolSendInput, sendDescription, sendSchema},
		{toolWait, waitDescription, waitSchema},
		{toolCloseAgent, closeDescription, closeSchema},
	} {
		var params map[string]any
		if err := json.Unmarshal([]byte(t.schema), &params); err != nil {
			panic(fmt.Sprintf("invalid %s schema: %v", t.name, err)) // a constant
		}
		defs = append(defs, tool.Definition{Tool: llm.Tool{Type: llm.ToolFunction, Name: t.name, Description: t.description, Parameters: params}})
	}
...
func (r agentRegistry) Resolve(name string) (tool.Translator, bool) {
	if t, ok := r.Registry.Resolve(name); ok || !slices.Contains(agentToolNames, name) {
		return t, ok
	}

	return agentTranslator{name: name, offered: r.offer}, true
}

// agentTranslator submits an agent tool call as a remote job and reads its
// result.
