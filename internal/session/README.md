...
`Compact` marks a compaction as pending. While a run is live, the engine compacts before its next model request; while idle, `engine.Options.Compact` asks the next run to compact first. The pending flag clears when the engine reports a manual `CompactionStarted`. The process engine returns `ErrNoCompaction`.
<!-- /memoria:section -->
