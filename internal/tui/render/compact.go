...
func compactLines(it state.Item, w int, now time.Time) ([]string, bool) {
	...
	case state.KindAssistant:
		mark := accent.Render("• ")
		if it.Final {
			return append([]string{""}, markdownLines(it.Text, w, mark, "  ")...), true
		}

		return markdownLines(it.Text, w, mark, "  "), true
...
func userLines(text, suffix string, w int) []string {
	lines := wrapPrefixed(text, w, accent.Render("λ "), "  ")
	lines[len(lines)-1] += suffix
	out := []string{"", band("", w)}
	for _, l := range lines {
		out = append(out, band(l, w))
	}

	return append(out, band("", w))
}
...
func finishLine(it state.Item) string {
	when := dim.Render(fmt.Sprintf("%s · worked %s", it.Started.Add(it.Wall).Local().Format("3:04 PM"), elapsed(it.Wall)))
	switch it.Status {
	case core.StatusOK, core.StatusRunning:
		return "  " + when
	case core.StatusInterrupted:
		return warn.Render("  ■ interrupted") + dim.Render(" · ") + when
	case core.StatusTimeout, core.StatusDiskLimit, core.StatusFailed:
	}

	return bad.Render(fmt.Sprintf("  ✗ run ended: %s", it.Status)) + dim.Render(" · ") + when
}
...
func compactTool(it state.Item, w int, now time.Time) string {
	...
	case state.ToolFailed:
		when = bad.Render(pad("fail")) + " "
	case state.ToolStopped:
		when = warn.Render(pad("stop")) + " "
	}
	var tail string
	if it.Tool == state.ToolFailed && it.Detail != "" {
		tail = bad.Render("  " + oneLine(it.Detail))
	}
	var head string
	switch {
	case live:
		head = accent.Render("  " + pad(label) + pad(when) + " ")
	case it.Tool == state.ToolFailed || it.Tool == state.ToolStopped:
		head = dim.Render("  "+pad(label)) + when
	default:
...
func agentLines(it state.Item, w int, now time.Time) []string {
	name := it.Name
	if it.Label != "" {
		name += dim.Render(" " + it.Label)
	}
	var head string
	switch it.Detail {
	...
	if it.Detail == engine.AgentRunning {
		out = append(out, ansi.Truncate(dim.Render("    └ ")+tool.Render(spin(now))+dim.Render(" "+agentDoing(it)), w, "…"))
	}
...
func banner(s state.State, version string, w int) []string {
	if version != "" {
		version = " (" + version + ")"
	}
	rows := []string{
		accent.Render("λ uah") + dim.Render(version),
		"",
		dim.Render("model:     ") + strings.TrimSpace(s.Settings.Model+" "+s.Settings.Effort) + dim.Render("   /model to change"),
		dim.Render("directory: ") + home(s.Settings.Workspace),
	}
	inner := 0
	for _, r := range rows {
	...
	out := []string{dim.Render("╭" + strings.Repeat("─", inner+2) + "╮")}
	for _, r := range rows {
		r = ansi.Truncate(r, inner, "…")
		out = append(out, dim.Render("│ ")+r+strings.Repeat(" ", inner-ansi.StringWidth(r))+dim.Render(" │"))
	}

	return append(out, dim.Render("╰"+strings.Repeat("─", inner+2)+"╯"), "")
}

// workingLine is the breathing λ and "Working (12s • esc to interrupt)";
// without a run (since is zero) only the λ and the verb.
func workingLine(now time.Time, verb string, since time.Time) string {
	line := breathing(now.UnixMilli()) + " " + bold.Render(verb)
	if since.IsZero() {
		return line
	}

	return line + dim.Render(fmt.Sprintf(" (%s • esc to interrupt)", elapsed(now.Sub(since))))
}
