...
<!-- memoria:section id="look" files="render/theme.go render/compact.go render/screen.go render/markdown.go render/markdown/markdown.go render/markdown/blocks.go render/markdown/inline.go render/markdown/table.go render/markdown/highlight.go bubble/model.go render/diff.go render/words.go state/context.go render/backtrack.go" -->
## The look
...
| Agent messages | `•` in the accent, then the Markdown: `code` in the code color, bold, italic, strikethrough, headings in bold, `•` bullets and dim numbers with a hanging indent, task lists as `[x]`, quotes behind a dim `│ `, dim rules, links as the text with the URL dim after it, and one blank line between blocks | `compactLines`, `markdownLines`; package `render/markdown` |
| Code blocks | On the band, highlighted with the theme's code colors, not wrapped; a fence without a known language in the code color | `codeBlock`, `highlight` in `render/markdown` |
| Tables | Codex's: columns two apart with no vertical rules, the header bold with a dim `━` rule under it and dim `─` rules between rows, cells wrapped to fit the width; too narrow for that, each row as records (`Header  value`) | `table.go` in `render/markdown` |
...
| Going back | The selected message as `▶ … ↵ edit from here` on the band; the rest of the transcript faded to the dim color, keeping only backgrounds | `transcriptLines`, `fade` in `render/backtrack.go` |
...
| `render/markdown/markdown_test.go`, `render/markdown/incremental_test.go`, `render/markdown_test.go`, `render/markdown_bench_test.go` | Markdown: every construct against goldens at several widths (`render/markdown/testdata`, `-update` rewrites them), tables down to records, highlighting and its cache, incremental rendering equal to full rendering for every prefix, each update parsing only the last block, each theme's colors, and the benchmarks (`go test -run '^$' -bench Markdown -benchmem ./internal/tui/render`) |
