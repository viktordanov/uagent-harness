...
func TestMenu_Commands(t *testing.T) {
	s := opened()
	assert.Equal(t, []string{"/resume [id]", "/reasoning"}, labels(s.Suggestions("/re")))
...
func TestMenu_MoveAcceptClose(t *testing.T) {
	...
	assert.Equal(t, []state.Effect{state.EffSetDraft{Text: "/reasoning"}}, effects)
	assert.Equal(t, 0, s.Menu.Index)

	s, _ = apply(s, state.MenuMove{Draft: "/re", Delta: -1})
	assert.Equal(t, 1, s.Menu.Index, "moving up from the top wraps")
