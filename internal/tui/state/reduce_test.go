...
func TestReduce_StatusNamesTheEngineGaps(t *testing.T) {
	...
	assert.Equal(t, "the process engine runs without: live input, live settings, fast mode, compaction, PreCompact hooks, command rules, "+
		"prompt rules, approvals, Auto mode, PermissionRequest hooks, PreToolUse hooks, MCP servers, subagents, apply_patch, Codex skills, /context, pasted images, reconnect status, streaming", last.Text)
	...
	all.Caps = engine.Capabilities{
		...
		Approvals: true, ToolHooks: true, MCP: true, Subagents: true, ApplyPatch: true, CodexSkills: true, ContextUsage: true, Images: true, Reconnect: true, Stream: true,
	}
