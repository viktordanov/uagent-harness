...
| Switching models on resume fails on replayed encrypted reasoning | Capture a real test before M4 ships `/model` for resumed sessions; strip foreign reasoning in a builder wrapper if needed |
| `service_tier` rejected by the Codex subscription backend | Probe once with a real request; hide `/fast` for that provider if rejected |
