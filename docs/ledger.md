...
| 48 | Select and copy inside the TUI while the mouse is reported: drag to select (the selection follows the transcript as it scrolls or streams), double-click a word, triple-click a line, copy on release to the system clipboard, and the wheel keeps scrolling | lane select | pending |
