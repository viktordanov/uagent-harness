...
| 45 | Streaming: the assistant's answer (and reasoning summaries where shown) appears in the TUI as the model writes it, not all at once when it finishes; the runner stays unchanged | lane stream | done (embedded engine: the transport tees each turn request's SSE body; the runner's final message stays authoritative; see docs/design/streaming.md) |
| 46 | Transcript editing, as Codex's backtrack: pick an earlier message of yours, edit it (or drop it), and continue from there; what came after leaves the model's context, and the old branch stays on disk | lane rewind | done (esc esc on an empty composer while idle, or `/rewind`; a cut record next to the session hides the cut items from the context builder, survives resume, and keeps the old branch in the session file; the process engine states the gap; files are not restored, see [the rewind design](design/rewind.md)) |
| 47 | Rich text as a core subsystem, designed for speed: a real Markdown parser (GFM: tables, lists, code, quotes, links), tables that fit the width, cached syntax highlighting, and incremental rendering so a streaming answer re-renders only its last block; benchmarks gate it | lane markdown | done (`internal/tui/render/markdown` on goldmark: finished top-level blocks kept per document, so a streaming update parses and draws only the last block, 30 µs instead of 1.16 ms on a 215-line answer; Codex's tables with records when too narrow; highlighting cached per block; `markdownLines` keeps its signature for the streaming lane; see [the design](design/markdown.md)) |
| 48 | Select and copy inside the TUI while the mouse is reported: drag to select (the selection follows the transcript as it scrolls or streams), double-click a word, triple-click a line, copy on release to the system clipboard, and the wheel keeps scrolling | lane select | done (on by default: `[tui] mouse` now defaults to true, `false` gives the terminal its selection back; positions are item keys and drawn lines, so a selection survives scrolling and streaming; drawn as a pass over the window like the backtrack fade; the copy is what is drawn minus the λ/• columns and band padding, through OSC 52 and pbcopy, wl-copy, or xclip; see [the selection design](design/selection.md)) |
...
- 16:30 · 43 · One home: everything uah reads and writes lives in `~/.uah` (`UAH_HOME` overrides it), project files in `<workspace>/.uah`. The first start without `~/.uah` copies `~/.config/uagent` and uah's part of `~/.local/state/unreal-agent` (sessions, runs, images, models, and the index through `VACUUM INTO`) into a temporary directory and renames it into place, leaving the old folders untouched. A found `.uagent`, `UAGENT_CONFIG`, or `UAGENT_STATE_DIR` gets a notice and a `uah doctor` warning; a test resumes a migrated session with a pasted image after the old folder is gone.
- 16:30 · 44 · `model_instructions_file` with Codex's semantics: the file's text replaces the base instructions (the runner's host prompt), a relative path resolves against the config file that sets it, and a missing or empty file stops the session. AGENTS.md follows it, and subagents and both engines get it; `/context` finds the instructions by the whole header. unreal-agent's context builder keeps its own preamble and skill list in front. `uah prompts init` writes `system.md` (the runner's text) and `system-codex.md` (Codex's gpt-6-sol template from models.json at rust-v0.156.1; the Markdown prompts in core are unused there) with its key line commented out; docs/configuration.md lists the tools Codex's prompt names that uah lacks. `uah doctor` checks the file.
- 17:37 · 45 · Streaming without changing the runner (docs/design/streaming.md): with `engine.Options.Stream`, the embedded engine's transport tees each turn request's SSE body into a parser, and the text and reasoning-summary deltas become `engine.TextDelta` and `ReasoningDelta`, emitted by a pump so the runner's read never waits and all of them before the response's final events. A retry, a failed or canceled request resets them (`StreamReset`). Compaction summaries, the auto-reviewer, and subagents never stream. The TUI grows `Streaming` items that the runner's `AssistantMessage` and `ReasoningSummary` replace in place, and the working line says `Writing`; `uah run --stream` adds `text_delta`, `reasoning_delta`, and `stream_reset`; plain `uah run` prints the final answer. The process engine lacks it (`streaming` in the engine table). `testing/fakellm` streams replies in deltas (`Deltas`, `Reasoning`, `Hold`).
- 17:45 · 46 · Going back to an earlier message, after Codex's backtrack (rust-v0.156.1): esc on an empty composer while idle primes it, a second esc or `/rewind` selects your latest message, esc/↑ and ↓ step, enter puts it back in the composer with its images and calls `Session.Rewind`, and `engine.Rewound` cuts the transcript. The embedded engine records the cut as runner item sequences in `sessions/<id>.rewind.jsonl`; every run's store leaves those items out (`cutStore`, which also chains new turns to the file's latest turn), so the next request carries only the history before the message. Inputs that went with the message are sent again; a message that arrived while a tool call had no status is refused. Compactions a rewind made stale fall back to the one before; `/context` and the meter take the cut's tokens; resume, `session.Load`, `uah sessions show`, and `--stream` keep it. The process engine states the gap (`FeatureRewind`).
- 17:47 · 47 · Markdown as a subsystem (`internal/tui/render/markdown`): goldmark with GFM and a renderer of uah's own walking the tree (glamour was 100 times slower and brings 39 modules). A `Renderer` per theme keeps each document's finished top-level blocks, so an update parses only the text after them, as Codex's streaming render does; link definitions draw the whole text. Tables after Codex (padding, gaps, `━`/`─` rules, columns shrunk to fit, records when too narrow); chroma lexers and highlighted blocks cached, an open fence's complete lines cached; styles carried across wrapped lines. On the 215-line fixture: one render 2.39 → 0.87 ms, the same text again 0.66 µs, a streaming update 1.16 ms → 30 µs. Tests gate the work (each update parses at most the previous last block onward), incremental equals full for every prefix, and CI runs the benchmarks once.
- 18:45 · 46 · Going back fades the rest: while a message is selected, everything else in the transcript, also what the cut would drop, is drawn in the theme's dim color, keeping only backgrounds such as the band. A pass over the window's lines each frame (`render/backtrack.go`), so the item cache is untouched and entering or leaving the selection renders no Markdown again.
- 23:39 · 47 · The Markdown styles the owner chose from the mocks (docs/design/markdown.md, "Styles chosen 2026-09-25"): code on the band with the language dim at the first line's right end and `diff` blocks on the edit tool's tints (B); zebra tables with an accent header and no rules, the records fallback banded to match (D); quotes indented between dim “ ” marks, GitHub alerts as a bold title in the alert's color (C); H1 in accent capitals, H2 in the accent, `◦` nested bullets, right-aligned numbers (D). Fixed the band re-applying itself after a color with a 0 component, which took the backtrack label's accent background. Benchmarks unchanged.
- 23:55 · 48 · Select and copy in the TUI (docs/design/selection.md): with the mouse reported, a drag over the transcript selects from cell to cell, a double click a word (a run of non-space), a triple click a line, and the release copies with OSC 52 (`tea.SetClipboard`) and the system's tool (`clipboard.TextWriter`: pbcopy, wl-copy, or xclip; none is fine), then the footer says `copied 3 lines`. The reducer anchors the selection in item keys, drawn lines, and cells (`state.TextPos`), so it stays on its text while the transcript scrolls or streams; each frame records which transcript line every window row shows (`render.Cache.At`) and draws the selection as a pass over the window's lines on the theme's new `Selection` background, leaving the cache alone. Dragging to the top row or below the transcript scrolls a line per move, and the wheel keeps scrolling during a drag. Esc, a click, typing, sending, ctrl+t, and ctrl+r clear it. `[tui] mouse` defaults to true now (`*bool`, override that can unset), since copying and wheel scrolling both work inside the TUI; `false` leaves the mouse to the terminal. Codex rust-v0.156.1 builds the same in its opt-in `[tui] fullscreen_transcript` view, copying on ctrl+c or a right click instead of on release.
